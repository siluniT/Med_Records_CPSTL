...
function ProfilePage() {
  ...
  const defaultPlaceholderImage = "https://placehold.co/150x150/E0E7FF/4338CA?text=User";
  const currentProfileImageSrc = profileImage || defaultPlaceholderImage;
  ...
  const DetailRow = ({ label, value }) => (
    <p className="flex justify-between items-start">
      ...
      <span className="text-gray-800 text-right flex-1 break-words">{value || 'N/A'}</span>
    </p>
  );
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gray-100 p-6">
        <div className="bg-white p-8 rounded-lg shadow-xl text-center">
          ...
          <p className="text-gray-600 mb-6">If you are not redirected, please log in.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans">
      <AppSidebar isSidebarOpen={isSidebarOpen} onCloseSidebar={() => setIsSidebarOpen(false)} currentPage="Profile" />
                ...
                <div className="w-full mt-6">
                  ...
                  <ul className="space-y-2 text-sm text-gray-700">
                    <li className="flex justify-between items-center p-2 bg-white rounded-md shadow-sm hover:bg-gray-100 cursor-pointer">
                      <span>Add Past Employment</span> <span className="text-blue-600 font-bold">1</span>
                    </li>
                    <li className="flex justify-between items-center p-2 bg-white rounded-md shadow-sm hover:bg-gray-100 cursor-pointer">
                      <span>Add Leave Request</span> <span className="text-blue-600 font-bold">3</span>
                    </li>
                    <li className="flex justify-between items-center p-2 bg-white rounded-md shadow-sm hover:bg-gray-100 cursor-pointer">
                      <span>Add Expense Report</span> <span className="text-blue-600 font-bold">1</span>
                    </li>
                    <li className="flex justify-between items-center p-2 bg-white rounded-md shadow-sm hover:bg-gray-100 cursor-pointer">
                      <span>Add Internal Employment</span> <span className="text-blue-600 font-bold">0</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="md:col-span-2 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-4 border border-gray-200 rounded-lg bg-gray-50 shadow-sm col-span-1 lg:col-span-2">
                  ...
                  <div className="space-y-2 text-sm text-gray-700">
                    <DetailRow label="EPF Number" value={userProfile.epfNumber || userProfile.epf} />
                    <DetailRow label="User ID" value={userProfile.userId || userProfile.username} />
                    <DetailRow label="Department" value={userProfile.department} />
                    <DetailRow label="Name" value={userProfile.Name || userProfile.Emp_name?.split(' ')[0] || 'N/A'} />
                  </div>
                </div>
                ...
                <div className="p-4 border border-gray-200 rounded-lg bg-gray-50 shadow-sm col-span-1 lg:col-span-2">
                  ...
                  <div className="space-y-2 text-sm text-gray-700">
                    <DetailRow label="Status" value={<span className="text-green-600 font-semibold">{userProfile.activestatus ? 'Active' : 'Inactive'}</span>} />
                    <DetailRow label="Role" value={userProfile.role_type || 'Administrator, Member, Project Manager'} />
                  </div>
                </div>
              </div>
            </div>

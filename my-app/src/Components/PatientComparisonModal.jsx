...
const PatientComparisonModal = ({ patient, isOpen, onClose }) => {
  ...
  const calculateTrend = (data, metric) => {
    ...
    if (isNaN(recent) || isNaN(previous) || previous === 0) return { trend: 'stable', percentage: 0 };
  ...
  const selectedMetricData = currentData?.map(item => ({
    period: activeView === 'monthly' ? item.month : item.year,
    value: selectedMetric === 'bp' ? parseInt(item.bp?.split('/')[0]) : parseFloat(item[selectedMetric]),
  })).filter(item => !isNaN(item.value)) || [];

  // Corrected Pie Chart Data Logic
  const pieData = (() => {
    const normalCount = currentData.filter(d => getHealthStatus(selectedMetric, d[selectedMetric]).status === 'success').length;
    const warningCount = currentData.filter(d => getHealthStatus(selectedMetric, d[selectedMetric]).status === 'warning').length;
    const dangerCount = currentData.filter(d => getHealthStatus(selectedMetric, d[selectedMetric]).status === 'danger').length;
  ...
  const COLORS = ['#22C55E', '#FBBF24', '#EF4444']; // Green, Yellow, Red
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentData.map((item, index) => {
                  ...
                  const trend = index > 0
                    ? calculateTrend(currentData.slice(index - 1, index + 1), selectedMetric)

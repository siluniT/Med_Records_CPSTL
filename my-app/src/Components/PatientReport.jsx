    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <Text style={styles.hospitalSubtitle}>Comprehensive Healthcare Services</Text>
        </View>
        ...
        <View style={styles.cardsGrid}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>
                {patient.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString() : v(null)}
              </Text>
            </View>
            <View style={styles.row}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>{patient.height ? `${patient.height} cm` : v(null, 'Not recorded')}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.label}>Weight</Text>
              <Text style={styles.value}>{patient.weight ? `${patient.weight} kg` : v(null, 'Not recorded')}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.label}>BMI</Text>
              <Text style={styles.value}>{patient.bmi || v(null, 'Not calculated')}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.label}>Waist</Text>
              <Text style={styles.value}>{patient.waist ? `${patient.waist} cm` : v(null, 'Not recorded')}</Text>
            </View>
          </View>
                ...
                {(patient.visionLeft || patient.visionRight) && (
                  <View style={styles.row}>
                    ...
                    <Text style={styles.value}>
                      ...
                      {patient.visionLeft && patient.visionRight ? ' / ' : ''}
                      {patient.visionRight ? `Right: ${patient.visionRight}` : ''}
                    </Text>
                  </View>
                )}
              </View>
            ) : (
              <Text style={styles.muted}>No recent vitals or lab values recorded.</Text>
            )}
          </View>
        </View>
        ...
        <View style={{ marginTop: 10 }}>
          <View style={[styles.card, { width: '100%' }]}>
            ...
            <Text>
              {medicalHistoryText.trim().length > 0 ? medicalHistoryText : v(null, 'No significant medical history reported.')}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.meta}>
            <Text>
              Report Ref: {patient.registrationNo || 'REG'}-{Date.now()} | Generated: {currentDate}
            </Text>
          </View>
        </View>
      </Page>

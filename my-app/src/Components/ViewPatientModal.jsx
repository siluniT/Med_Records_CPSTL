...
const ViewPatientModal = ({ patient, isOpen, onClose }) => {
  ...
  const exportToPDF = async () => {
    try {
      ...
      const showVitals = !!patient.bp || !!patient.rbs || !!patient.fbs || !!patient.visionLeft || !!patient.visionRight;

      const v = (x, fallback = '—') =>
        x !== undefined && x !== null && String(x).trim() !== '' ? x : `<span class="muted">${fallback}</span>`;
...
<div class="hospital-subtitle">Comprehensive Healthcare Services</div>
      ...
      <div class="cards-2col">
        <div class="card">
          ...
          <div class="row"><div class="label">Patient Name</div><div class="value">${v(patient.name)}</div></div>
          <div class="row"><div class="label">Registration No.</div><div class="value">${v(patient.registrationNo)}</div></div>
          <div class="row"><div class="label">EPF No.</div><div class="value">${v(patient.epfNo)}</div></div>
          <div class="row"><div class="label">Status</div>
            <div class="value">
              <span class="chip ${
                patient.status === 'Deactivate' ? 'inactive' :
                patient.status === 'In treatment' ? 'treatment' : 'active'
        ...
        <div class="card">
          ...
          <div class="row"><div class="label">Date of Birth</div><div class="value">${
            patient.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString() : '<span class="muted">—</span>'
          }</div></div>
          <div class="row"><div class="label">Age</div><div class="value">${age ? age + ' years' : '<span class="muted">—</span>'}</div></div>
          <div class="row"><div class="label">Gender</div><div class="value">${v(patient.gender)}</div></div>
          <div class="row"><div class="label">Contact No.</div><div class="value">${v(patient.contactNo)}</div></div>
        </div>
      </div>
      ...
      <div class="cards-2col" style="margin-top:10px;">
        <div class="card">
          ...
          <table class="metrics">
            <tr>
              <td>Height</td><td>${patient.height ? patient.height + ' cm' : '<span class="muted">Not recorded</span>'}</td>
              <td>Weight</td><td>${patient.weight ? patient.weight + ' kg' : '<span class="muted">Not recorded</span>'}</td>
            </tr>
            <tr>
              <td>Body Mass Index (BMI)</td><td>${patient.bmi || '<span class="muted">Not calculated</span>'}</td>
              <td>Waist Circumference</td><td>${patient.waist ? patient.waist + ' cm' : '<span class="muted">Not recorded</span>'}</td>
            </tr>
          </table>
        </div>
        ...
        ${
          ...
          <div class="card">
            ...
            <table class="metrics">
              ${patient.bp ? `<tr><td>Blood Pressure</td><td colspan="3">${patient.bp} mmHg</td></tr>` : ''}
      ...
      <div class="cards-1col" style="margin-top:10px;">
        <div class="card">
          ...
          <div>${(medicalHistoryText && medicalHistoryText.trim().length > 0) ? medicalHistoryText : '<span class="muted">No significant medical history reported.</span>'}</div>
        </div>
      ...
      <div class="footer">
        ...
        <div class="meta">
          <strong>Report Ref:</strong> ${patient.registrationNo || 'REG'}-${Date.now()} |
